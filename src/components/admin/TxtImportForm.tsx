...
export function TxtImportForm({ tiers, defaultTierId }: { tiers: Tier[]; defaultTierId?: string }) {
  const [state, formAction, pending] = useActionState<ActionState, FormData>(importArtworksTxtAction, undefined);

  return (
    <details className="bg-white border border-neutral-200 rounded-xl p-4" open={!!defaultTierId}>
